...
/** Our own channel version, encoded into iLink-App-ClientVersion. */
const CHANNEL_VERSION = "1.0.0";

/** iLink-App-Id: fixed to "bot" (matches the upstream plugin's package.json field). */
const ILINK_APP_ID = "bot";

/** iLink-App-ClientVersion: uint32 as 0x00MMNNPP. */
...
/**
 * Sanitize a botAgent config value into a wire-safe UA-style string.
 * Tokens that fail to parse are dropped silently.
 */
...
export function classifyFetchError(err: unknown): {
  ...
  if (/ENOTFOUND|EAI_AGAIN|getaddrinfo/i.test(causeStr)) {
    return { type: "dns", description: "DNS resolution failed", ...(matchedCode ? { code: matchedCode } : {}) };
  }
...
export async function apiGetFetch(params: {
  ...
  try {
    ...
    if (t !== undefined) clearTimeout(t);
    logger.error(`${params.label}: GET fetch failed url=${redactUrl(url.toString())} err=${String(err)}`);
    throw err;
  }
}
...
export async function apiPostFetch(params: {
  ...
  const { signal, cleanup } = combineAbortSignals({ internal: controller, external: params.abortSignal });
  try {
    const res = await fetch(url.toString(), {
      method: "POST",
      headers: hdrs,
    ...
    logger.error(
      `${params.label}: POST fetch failed url=${redactUrl(url.toString())} type=${classified.type} err=${String(err)}`,
    );
    throw err;
  } finally {
    cleanup();
